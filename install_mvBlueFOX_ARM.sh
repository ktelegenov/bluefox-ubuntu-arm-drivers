...
if [ "$(ls | grep -c "$PACKAGENAME"'.*\.tgz')" != "0" ]; then
    ...
    if [ "$(echo "$TARNAME" | grep -c ARMhf)" != "0" ]; then
        ...
        echo "In case the file has been renamed, please revert to original name."
        echo "Terminating installation script now..."
        echo
        exit 1
    fi
    TARFILE="$(ls "$PACKAGENAME"*.tgz | tail -1)"
    VERSION="$(ls "$PACKAGENAME"*.tgz | tail -1 | sed -e s/\\"$PACKAGENAME"// | sed -e s/\\-"$TARGET"_gnu.*-// | sed -e s/\\.tgz//)"
...
ACT=mvIMPACT_Acquire-$VERSION.tar
...
# Clean up /tmp
rm -r -f /tmp/$ACT2 /tmp/mvIMPACT_Acquire-$VERSION
...
if [ $TARGET != "ARM64" ]; then
    ...
    # (eg. armv7l, arm7ahf etc. ) and will not be 'armhf', a softlink has to be created,
...
if [ "$MINIMAL_INSTALLATION" == "NO" ]; then
    if ([ "$REPOSITORY_ACCESS_AVAILABLE" == "YES" ] || [ "$FORCE_REPOSITORY_USAGE" == "YES" ]) || [ "$USE_DEFAULTS" == "YES" ]; then
        ...
        if [ "$TARGET" != "armv7ahf" ]; then
            ...
            echo
            if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
                echo 'The tools and samples will not be built.'
                echo 'To build them manually, type:'
                ...
                if [ "$(which g++)" != "" ]; then
                    ...
                    echo "(which may require an Internet connection!)."
                    echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
                    if [ "$USE_DEFAULTS" == "NO" ]; then
                        read YES_NO
                    else
                    ...
                    if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
                        ...
                        if [ "$REPOSITORY_ACCESS_AVAILABLE" == "YES" ] || [ "$FORCE_REPOSITORY_USAGE" == "YES" ]; then
                            ...
                            if [ "$(wx-config --release 2>&1 | grep -c "^3.")" != "1" ] ||
                                ...
                                if [ "x$(which apt-get)" != "x" ]; then
                                    ...
                                    if [ $? == 0 ] && [ "x$(which wx-config)" != "x" ]; then
                                        echo "Necessary wxWidgets libraries successfully installed!"
                                        echo
                                    else
                                        echo "wxWidgets libraries could not be downloaded and installed automatically on this system!"
                            ...
                            echo "No repository seems to be available!"
                            echo
                            echo "Please make sure the repositories are available and re-run this installer script,"
                            echo "or re-run this script and choose not to build the wxWidgets GUI tools altogether!"
                            echo
                        fi
                    fi
